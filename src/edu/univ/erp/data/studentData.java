    ...
    public static String fetchRollNo() throws SQLException{
        ...
        try
            (
                ...
                Statement statement = connect.createStatement();
                ResultSet result = statement.executeQuery(String.format("select roll_no from students where user_id = %s", Session.getCurrentUser_ID()));
            )
        {
            while (result.next()){
    ...
    public static String fetchEmailId() throws SQLException{
        ...
        try
            (
                Connection connect = HikariConnectionPool.getDataSource().getConnection();
                Statement statement = connect.createStatement();
                ResultSet result = statement.executeQuery(String.format("select email_id from students where user_id = %s", Session.getCurrentUser_ID()));
            )
        {
            while (result.next()){
                tmp = result.getString(1);
    ...
    public static HashMap<String, Double> getCourseComponentGrades(String Course_ID) throws SQLException{
        ...
        try
            (
                Connection connect = HikariConnectionPool.getDataSource().getConnection();
                Statement statement = connect.createStatement();
                ResultSet result = statement.executeQuery(String.format("select distinct grades.component, grades.score, grades.total_score, grades.weightage from grades, enrollments join sections on sections.section_id = grades.section_id where enrollments.student_id = '%s' and sections.course_id = '%s';", Session.getCurrentUser_ID(), Course_ID));
            )
        {
            while (result.next() != false){
